...
const PLATFORM: CategoryDescriptor<keyof PlatformModules> = {
  ...
  leaves: {
    ...
    userMetadata:     { label: 'User metadata',      description: 'Schema admin for custom per-user metadata fields at /users/metadata.', icon: IdCard },
    publicDispatcher: { label: 'Public dispatcher',  description: 'Single /pub/[token] entrypoint that routes to forms or pages.',        icon: Link2 },
  },
};
...
const APP: CategoryDescriptor<keyof AppModules> = {
  ...
  leaves: {
    ...
    appSidebar:               { label: 'App sidebar',              description: 'App-scoped sidebar nav generated from pages + role filtering. Coexists with the tabs nav.', icon: PanelLeft },
    dashboards:               { label: 'Dashboards',               description: 'First-class dashboards with list, viewer, and builder routes.', icon: LayoutDashboard },
    customComponentLifecycle: { label: 'Component lifecycle UI',   description: 'Versions, usage, share, rollback, and diff on the component editor.', icon: History },
    complexityDrawer:         { label: 'Complexity drawer',        description: 'Drawer variant of the App Complexity score (replaces the modal).', icon: Gauge },
    csvImportStepper:         { label: 'CSV import wizard',        description: 'Multi-step CSV import with validation preview and error report.', icon: Upload },
    formTemplates:            { label: 'Form templates',           description: 'Template picker on the "New form" flow (blank vs. starter).',  icon: ClipboardList },
  },
};
...
const SERVICES: CategoryDescriptor<keyof ServiceModules> = {
  ...
  leaves: {
    ...
    aiAssistant:     { label: 'AI Assistant drawer',  description: 'In-app AI chat drawer that calls schema/data/forms tool endpoints.', icon: MessageSquare },
    aiAppBuilder:    { label: 'AI App Builder drawer',description: 'In-app drawer that modifies the current app via tool calls.',     icon: Wand2 },
    pageSdkExtended: { label: 'Extended page SDK',    description: 'Extra iframe SDK methods (relationships, documents, form widget channel, 30s timeout).', icon: Plug },
  },
};
...
const PAGE_BUILDER: CategoryDescriptor<keyof PageBuilderModules> = {
  ...
  leaves: {
    ...
    verticalWidgets:    { label: 'Vertical widgets',      description: 'Lazy-loaded domain widgets (HealthScorecard, BudgetWaterfall, …).',  icon: Boxes },
    convertToComponent: { label: 'Convert to component',  description: '"Convert to component" wizard turns a block into a reusable component.', icon: Replace },
    pageShare:          { label: 'Page share',            description: 'Share button on the editor toolbar (opens public-links create dialog).', icon: Share2 },
    codeProp:           { label: 'Code prop editor',      description: '"code" PropDefinition editor with inline AI code generation dialog.',  icon: Code2 },
  },
};
