...
/**
 * A searchable dropdown combobox with OpenGov-aligned shadows and transitions.
 *
 * @example
 * <Combobox
  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      <button
        ...
        id="combobox-trigger"
        className={cn(
          'flex w-full items-center justify-between rounded border border-border bg-background px-3 py-2 text-sm transition-all duration-300 ease-in-out',
      ...
      {open && (
        <div className="absolute z-[var(--z-dropdown)] mt-1 w-full rounded-xl border border-border bg-card shadow-medium overflow-hidden">
          {searchable && (
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((option) => (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center gap-2 rounded px-3 py-2 text-sm text-foreground hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring transition-all duration-300 ease-in-out',
